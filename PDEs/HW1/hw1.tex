...
\textcolor{blue}{\textbf{Solution} — The Fourier series of a $2\pi$-periodic function on $[-\pi,\pi]$ is given by $$f(x) = \frac{1}{2} a_0 + \sum_{n=1}^{\infty} a_n \cos(nx) + \sum_{n=1}^{\infty} b_n \sin(nx)$$ where $$a_0 = \frac{1}{\pi} \int_a^b f(x)dx \;\;\;\;\;\;\;a_n = \frac{1}{\pi} \int_a^b f(x)\cos(nx) dx$$ \\ $$b_n = \frac{1}{\pi} \int_a^b f(x) \sin(nx) dx$$ \\ \\ 
...
\end{align*}
So the value of $c$ that minimizes $D(c)$ happens to be exactly the average value of $f(x)$ over $[-\pi, \pi]$, as predicted. We verify with the second-derivative test:
\begin{align*}
&=  \frac{d^2 D}{dc} = \frac{d}{dc} \left( 2 \int_{-\pi}^{\pi} (c - f(x)) dx \right) \\
&= 4\pi > 0 \;\;\;\; \text{for all $c$}
\end{align*}
}
...
&= 2 \left[-\int_{-\pi}^\pi  f(x)\sin(x) dx + 0 + \pi b + 0 \right] = 0 \\
