...
\textcolor{blue}{\textbf{Solution —} We use the Fourier convergence theorem, which asserts that for a function $f \in PC^1[a,b]$, its Fourier series shall converge to the midpoint $(f(a^+) + f(a^-)/2$ of the left and right limits at each point $x=a$. \\ \\ Since $f(x) \in PC^1[-\pi/2, \pi/2]$ and $f(-\pi/2) = f(\pi/2)$ its Fourier series $F(x)$ is exactly the periodic extension of $f(x)$ to all $x\in\mathbb{R}$: 
